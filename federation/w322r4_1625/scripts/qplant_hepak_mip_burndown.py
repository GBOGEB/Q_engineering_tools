...
def validate_bt1(path: Path) -> tuple[bool, str]:
    ...
    for index, segment in enumerate(segments, start=1):
        ...
        if not _contains_numeric(segment.get("valve_K_or_Cv"), positive=True):
            return False, (
                f"segment #{index} valve_K_or_Cv must contain a finite positive value"
            )
        if not _contains_numeric(
...
def validate_bt2(path: Path) -> tuple[bool, str]:
    """Validate the W57 per-QCELL B-flow source/provenance population."""
    required = {
        "qcell_id",
        "installed_or_future",
    ...
    if missing:
        return False, f"missing W57 B-flow columns: {sorted(missing)}"
    if not rows:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        configuration = row["configuration"].strip().lower()
        identity = (qcell_id, mode, configuration)
        if identity in identities:
            return False, f"duplicate QCELL/mode/configuration row: {identity}"
    ...
    for (mode, configuration), expected_qcells in required_cohorts.items():
        cohort_qcells = {
            qcell_id
            for qcell_id, row_mode, row_configuration in normalized_rows
            if row_mode == mode and row_configuration == configuration
        }
        if len(cohort_qcells) < expected_qcells:
            return (
                False,
                f"{mode}/{configuration} population incomplete: "
                f"{len(cohort_qcells)} unique QCELL IDs; need >={expected_qcells}",
            )

    return (
        True,
        f"validated {len(rows)} W57 source-bound rows across required "
        "2K_OP/2K_SB config24/config30 cohorts",
    )
