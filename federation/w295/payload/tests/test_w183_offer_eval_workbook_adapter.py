...
def test_equation_v1_confidence_na_gate_and_risk_once(tmp_path):
    ...
    pairwise_total = wb["DASHBOARD"]["B3"].value
    diagnostic_total = wb["DASHBOARD"]["B6"].value
    ...
    assert "COUNT(CALC_AB!D2:D51)<50" in pairwise_total
    assert "COUNTA(CALC_AB!D2:D51)" not in pairwise_total
    assert "COUNT(CALC_AB!I2:I51)<50" in diagnostic_total
    assert "COUNTA(CALC_AB!I2:I51)" not in diagnostic_total
