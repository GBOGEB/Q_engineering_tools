#!/usr/bin/env python3
"""Generate exactly eight compact cluster reviewer one-pagers from the governed doctrine.
...
def configure_compact_page(doc: Document) -> None:
...
def fixed_payload(d: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "bidder_A_score_confidence_notes": "Reviewer input: Applicant A raw score, confidence, source/evidence note. No cross-bidder substitution.",
        "bidder_B_score_confidence_notes": "Reviewer input: Applicant B raw score, confidence, source/evidence note. No cross-bidder substitution.",
    }
...
def build(doctrine_path: Path, out_dir: Path) -> dict[str, Any]:
    ...
    for cid in EXPECTED_CLUSTERS:
        info = clusters[cid]
        doc = Document()
        configure_compact_page(doc)
        doc.add_heading(f"Reviewer One-Pager — {cid} {info['name']}", level=0)
        meta = doc.add_paragraph()
        meta.paragraph_format.space_after = Pt(2)
        m1 = meta.add_run(f"Doctrine SHA-256: {dsha}\n")
        m1.font.size = Pt(7.5)
        m2 = meta.add_run("Classification: DOWNSTREAM_REVIEW_VIEW_ONLY | Formal credit: 0")
        m2.font.size = Pt(7.5)
        m2.bold = True

        add_block(doc, "Score scale", fixed["score_scale"])
        add_block(doc, "Confidence scale", fixed["confidence_scale"])
        add_block(doc, "Scoring interpretation", fixed["scoring_interpretation_note"])
        ...
        p = doc.add_paragraph("Cross-cluster cues carry no duplicate weight credit.")
    ...
    return {
        ...
        "physical_one_page_contract": "PENDING_RENDER_COUNT_PROOF",
