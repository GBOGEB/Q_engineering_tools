...
class HepakMIPBurndownTests(unittest.TestCase):
    ...
    def test_self_consistent_receipt_without_trust_anchor_does_not_convert_bt0(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = evaluate(root)
            self.assertFalse(result["blockers"]["BT0_HEPAK"]["converted"])
            self.assertIn("source anchor", result["blockers"]["BT0_HEPAK"]["reason"])
    ...
    def test_repo_local_active_anchor_never_promotes_bt0_authority(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            self.assertFalse(result["blockers"]["BT0_HEPAK"]["converted"])
            self.assertIn(
                "EXTERNAL_OWNER_SOURCE_AUTHORITY_REQUIRED",
    ...
    def test_underflow_nonzero_residual_requires_tolerance_bt2(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            ...
            self._write_w57_bflow(path, complete=True)
            rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
            rows[0]["mass_balance_residual"] = "1e-400"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(rows)
    ...
    def test_decimal_above_exact_tolerance_does_not_round_down_bt2(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            path = receipts / "lineb_qcell_bflow_current.csv"
            self._write_w57_bflow(path, complete=True, tolerance=1.0)
            rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
            rows[0]["mass_balance_residual"] = "1.0000000000000001"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(rows)
    ...
    def test_decimal_context_does_not_round_residual_magnitude_bt2(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            ...
            self._write_w57_bflow(path, complete=True, tolerance=1.0)
            rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
            rows[0]["mass_balance_residual"] = "1.00000000000000000000000000001"
            rows[0]["tolerance"] = "1.00000000000000000000000000000"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(rows)
    ...
    def test_qualified_placeholder_locator_does_not_convert_bt2(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            ...
            self._write_w57_bflow(path, complete=True)
            rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
            rows[0]["source_locator"] = "pending://W56"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(rows)
    ...
    def test_placeholder_source_or_deferred_authority_does_not_convert_bt2(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            ...
            self._write_w57_bflow(path, complete=True)
            rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
            rows[0]["source_locator"] = "placeholder"
            rows[0]["authority_class"] = "DEFERRED"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(rows)
    ...
    def test_tiny_nonzero_residual_without_tolerance_does_not_convert_bt2(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            ...
            self._write_w57_bflow(path, complete=True)
            rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
            rows[0]["mass_balance_residual"] = "5e-10"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(rows)
    ...
    def test_residual_above_tolerance_has_no_serialization_grace(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            ...
            self._write_w57_bflow(path, complete=True, tolerance=0.005)
            rows = list(csv.DictReader(path.open(newline="", encoding="utf-8")))
            rows[0]["mass_balance_residual"] = "0.0050000005"
            with path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=rows[0].keys())
                writer.writeheader()
                writer.writerows(rows)
    ...
    def _write_bt3_geometry(self, receipts: Path, segment_ids: tuple[str, ...]) -> None:
        segments = []
        for index, segment_id in enumerate(segment_ids):
            segments.append(
                {
                    "segment_id": segment_id,
                    "drawing_or_offer_locator": f"P&ID-{segment_id}",
                    "revision": "C",
                    "material": "316L",
                    "DN_or_OD": "DN150",
                    "wall_or_schedule": "2.77 mm",
                    ...
                    "fittings_and_branch_local_losses": {"K_total": 0.0},
                    "valve_K_or_Cv": {"K": 0.0},
                    "QCELL_branch_position": {"chainage_m": index * 12.5},
                }
            )
        (receipts / "lineb_selected_geometry_current.json").write_text(
    ...
    def test_hepak_thermal_row_converts_bt3_when_geometry_population_matches(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            self._write_bt3_geometry(receipts, ("LB-1",))
            (receipts / "lineb_spatial_thermal_field_current.csv").write_text(
                (
                    "segment_id,heat_leak_W_m_or_discrete_W,inlet_T_K,inlet_h_J_kg,"
                    "outlet_T_K,outlet_h_J_kg,phase,rho_kg_m3,mu_Pa_s,provider,"
    ...
    def test_partial_thermal_segment_population_does_not_convert_bt3(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            self._write_bt3_geometry(receipts, ("LB-1", "LB-2"))
            (receipts / "lineb_spatial_thermal_field_current.csv").write_text(
                (
                    "segment_id,heat_leak_W_m_or_discrete_W,inlet_T_K,inlet_h_J_kg,"
                    "outlet_T_K,outlet_h_J_kg,phase,rho_kg_m3,mu_Pa_s,provider,"
            ...
            self.assertFalse(evaluate(root)["blockers"]["BT3_THERMAL_FIELD"]["converted"])
    ...
    def test_sline_psv_without_specific_locator_does_not_convert_bt4(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            (receipts / "sline_applicant_recovery_current.json").write_text(
                json.dumps(
                    {
                        "applicants": [
                            {"name": "ALAT", "active_recovery_capacity_g_s": 100.0, "start_delay_s": 3.0, "initial_inventory_kg": 500.0, "initial_temperature_K": 300.0, "routing_sequence": ["LINE_S", "RECOVERY", "WSH"], "source_locator": "ALAT-RECOVERY-REV-A"},
                            {"name": "LKT", "active_recovery_capacity_g_s": 100.0, "start_delay_s": 3.0, "initial_inventory_kg": 500.0, "initial_temperature_K": 300.0, "routing_sequence": ["LINE_S", "RECOVERY", "WSH"], "source_locator": "LKT-RECOVERY-REV-A"},
                        ],
                        "pressure_hierarchy_bara": {"minimum": 1.05, "nominal": 1.10, "maximum_current_protected_interface": 1.30},
                        "psv": {"tag": "PSV-QPS-001", "set_pressure_bara": 1.70, "reseat_pressure_bara": 1.55, "discharge_destination": "RECOVERY_HEADER"},
                        "source_locators": ["ALAT recovery note"],
                    }
                ),
                encoding="utf-8",
            )
            self.assertFalse(evaluate(root)["blockers"]["BT4_SLINE_RECOVERY"]["converted"])

    def test_unrelated_psv_locator_does_not_convert_bt4(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
                ...
                json.dumps(
                    {
                        ...
                        "pressure_hierarchy_bara": {"minimum": 1.05, "nominal": 1.10, "maximum_current_protected_interface": 1.30},
                        "psv": {"tag": "PSV-QPS-001", "set_pressure_bara": 1.70, "reseat_pressure_bara": 1.55, "discharge_destination": "RECOVERY_HEADER", "source_locator": "unrelated document"},
                        "source_locators": ["RTM-242/243", "P&ID-PSV-QPS-001"],
                    }
                ),
                encoding="utf-8",
            )
    ...
    def _write_sline_with_psv_locator(self, receipts: Path, locator: str) -> None:
        (receipts / "sline_applicant_recovery_current.json").write_text(
            json.dumps(
                {
                    "applicants": [
    ...
    def test_psv_locator_tag_substring_does_not_convert_bt4(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            self._write_sline_with_psv_locator(
                receipts,
                "PSV-QPS-0010 / set 1.7 bara / reseat 1.55 bara",
            )
            self.assertFalse(evaluate(root)["blockers"]["BT4_SLINE_RECOVERY"]["converted"])

    def test_psv_locator_swapped_labeled_pressures_does_not_convert_bt4(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            self._write_sline_with_psv_locator(
                receipts,
                "PSV-QPS-001 / set 1.55 bara / reseat 1.7 bara",
            )
            self.assertFalse(evaluate(root)["blockers"]["BT4_SLINE_RECOVERY"]["converted"])

    def test_psv_locator_embedded_pressure_substrings_do_not_convert_bt4(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            receipts = root / "receipts"
            receipts.mkdir()
            self._write_sline_with_psv_locator(
                receipts,
                "PSV-QPS-001 / set 11.7 bara / reseat 21.55 bara",
            )
            self.assertFalse(evaluate(root)["blockers"]["BT4_SLINE_RECOVERY"]["converted"])
                ...
                json.dumps(
                    {
                        ...
                        "psv": {
                            ...
                            "reseat_pressure_bara": 1.55,
                            "discharge_destination": "RECOVERY_HEADER",
                            "source_locator": "P&ID-PSV-QPS-001 / PSV-QPS-001 / set 1.7 bara / reseat 1.55 bara",
                        },
                        "source_locators": [
                            "RTM-242/243",
                            "P&ID-PSV-QPS-001",
