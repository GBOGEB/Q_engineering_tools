...
class UniqueKeyLoader(yaml.SafeLoader):
    """SafeLoader variant that rejects YAML-equivalent duplicate mapping keys."""
...
def construct_unique_mapping(
    ...
    if not isinstance(node, MappingNode):
        raise ValidationError("expected_mapping_node", node.id)
    ...
    for key_node, value_node in node.value:
        ...
        try:
            ...
            raise ValidationError("unhashable_yaml_key", key) from exc
        if duplicate:
...
def load_unique_yaml(text: str, label: str) -> dict:
    try:
    ...
    if not isinstance(document, dict):
        raise ValidationError("yaml_document_not_mapping", label)
    return document
...
def validate(
    ...
    # Parse both published YAML surfaces with semantic duplicate-key rejection.
    # This catches quoted keys, flow mappings, comments and other YAML-equivalent
    # spellings that literal marker counts cannot recognize.
    load_unique_yaml(summary_text, "summary")
    load_unique_yaml(validation_text, "validation")
...
def main() -> int:
    ...
    if args.self_test:
        ...
        flow_shadowed_summary = summary_text + (
        ...
        expect_rejected(
            ...
            lambda: validate(
                ...
                register_rows,
                membership_rows,
                flow_shadowed_summary,
                validation_text,
            ),
        )

        quoted_shadowed_validation = validation_text.replace(
            "    VP-SAT-02:\n",
            (
                "    'VP-SAT-01': {members: [RTM-510], "
                "expected_sections: [4.13.3.1], result: PASS}\n"
                "    VP-SAT-02:\n"
            ),
            1,
        )
