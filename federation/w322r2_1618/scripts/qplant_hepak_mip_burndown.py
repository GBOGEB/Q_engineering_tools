...
def validate_bt2(path: Path) -> tuple[bool, str]:
    required = {"qcell_id", "mode", "configuration", "B_flow_g_s", "source_locator"}
    if not path.exists():
        return False, "missing receipt"
    try:
    ...
    for index, row in enumerate(rows, start=1):
        for field in ("qcell_id", "mode", "configuration", "source_locator"):
    ...
    for (mode, configuration), expected_qcells in required_cohorts.items():
        cohort_qcells = {
            row["qcell_id"].strip()
            for row in rows
            if row["mode"].strip() == mode
            and row["configuration"].strip() == configuration
        }
    ...
    return (
        True,
        f"validated {len(rows)} rows across required 2K_OP/2K_SB CONFIG24/CONFIG30 cohorts",
    )
