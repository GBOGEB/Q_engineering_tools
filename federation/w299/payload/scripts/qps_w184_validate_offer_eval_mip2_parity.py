...
def validate(repo_root: Path, workdir: Path) -> dict[str, Any]:
    ...
    if 'IF(NOT(ISNUMBER(E2)),E2,IF(J2=""' not in row_state_a:
        raise AssertionError("row state must preserve nonnumeric risk state before score completeness")
    if "NOT(ISNUMBER(F2))" not in diagnostic:
        raise AssertionError("diagnostic consumer must guard propagated nonnumeric risk state")
    cluster_contribution_a = wb["CALC_AB"]["T2"].value or ""
    if "NOT(ISNUMBER(F2))" not in cluster_contribution_a or "NOT(ISNUMBER(R2))" not in cluster_contribution_a:
        raise AssertionError("cluster contribution must guard propagated nonnumeric risk state")
