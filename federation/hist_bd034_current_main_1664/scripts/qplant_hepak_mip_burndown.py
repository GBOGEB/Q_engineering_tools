...
_PROVENANCE_FORBIDDEN_TOKEN_RE = re.compile(
    r"(?:^|[^a-z0-9])(?:fixture|synthetic|placeholder|test)(?:$|[^a-z0-9])"
)
    ...
    *,
    ...
    if source_file is not None:
        ...
        if relative_source.is_absolute():
            return False, "trusted HEPAK anchor source_file must be relative"
        root_resolved = root.resolve()
        candidate = (root / relative_source).resolve()
        try:
            candidate.relative_to(root_resolved)
        except ValueError:
            return False, "trusted HEPAK anchor source_file escapes governed root"
...
def _validate_bt0_candidate(root: Path) -> tuple[bool, str]:
    ...
    if _has_forbidden_provenance_token(source_workbook):
        return False, "test/synthetic source workbook is not production evidence"
    if _has_forbidden_provenance_token(runtime_host) or runtime_host.upper().startswith("TEST"):
        return False, "test/synthetic runtime host is not production evidence"
    ...
    for row, (state_id, (expected_t, expected_p)) in zip(
        ...
        for provenance_field in ("reason", "source_workbook", "runtime_host"):
            if _has_forbidden_provenance_token(row.get(provenance_field)):
                return False, (
                    f"{state_id}: synthetic/test {provenance_field} provenance "
                    "is not admissible"
                )
        if str(row.get("runtime_host", "")).upper().startswith("TEST"):
            return False, f"{state_id}: test runtime host is not admissible"
    ...
    return True, f"validated BT0 candidate receipt with {len(rows)} governed rows"


def validate_bt0(root: Path) -> tuple[bool, str]:
    """Validate candidate integrity but never promote repo-local evidence to owner authority."""
    candidate_ok, reason = _validate_bt0_candidate(root)
    if not candidate_ok:
        return False, reason
    return (
        False,
        "BT0 candidate structurally valid; EXTERNAL_OWNER_SOURCE_AUTHORITY_REQUIRED; "
        "repo-local evidence cannot promote physical conversion",
    )
...
def _has_forbidden_provenance_token(value: Any) -> bool:
    return isinstance(value, str) and bool(
        _PROVENANCE_FORBIDDEN_TOKEN_RE.search(value.strip().lower())
    )


def _substantive_source_locator(value: Any) -> bool:
    if not _nonempty_text(value):
        return False
    normalized = value.strip().lower()
    forbidden = {
        "placeholder", "tbd", "todo", "unknown", "none", "null",
        "n/a", "na", "pending", "deferred",
    }
    if normalized in forbidden or _has_forbidden_provenance_token(value):
        return False
    tokens = {
        token
    ...
    return not bool(
        tokens
        & {"placeholder", "tbd", "todo", "unknown", "none", "null", "pending", "deferred"}
    )


def _substantive_bt2_source_locator(value: Any) -> bool:
    if not _substantive_source_locator(value):
        return False
    normalized = value.strip().lower()
    tokens = {
        token
...
def _source_authority_valid(value: Any) -> bool:
    ...
    return normalized in BT2_SOURCE_BEARING_AUTHORITY_CLASSES
...
def _psv_locator_matches(
    locator: Any,
    *,
    tag: Any,
    set_pressure_bara: Any,
    reseat_pressure_bara: Any,
) -> bool:
    if not _substantive_source_locator(locator) or not _nonempty_text(tag):
        return False
    set_pressure = _finite_decimal(set_pressure_bara)
    reseat_pressure = _finite_decimal(reseat_pressure_bara)
    if set_pressure is None or reseat_pressure is None:
        return False

    text = str(locator).strip()
    tag_text = str(tag).strip()

    # Require the tag as an exact boundary-delimited field rather than a substring.
    fields = [part.strip() for part in re.split(r"[|/;,]+", text) if part.strip()]
    if tag_text not in fields:
        return False

    number = r"[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?"
    set_match = re.search(
        rf"(?i)(?:^|[|/;,]\s*)set(?:_pressure)?(?:_bara)?\s*[:=]?\s*({number})\s*(?:bara)?(?:\s*$|\s*[|/;,])",
        text,
    )
    reseat_match = re.search(
        rf"(?i)(?:^|[|/;,]\s*)reseat(?:_pressure)?(?:_bara)?\s*[:=]?\s*({number})\s*(?:bara)?(?:\s*$|\s*[|/;,])",
        text,
    )
    if not set_match or not reseat_match:
        return False

    try:
        locator_set = Decimal(set_match.group(1))
        locator_reseat = Decimal(reseat_match.group(1))
    except InvalidOperation:
        return False
    return locator_set == set_pressure and locator_reseat == reseat_pressure
...
def validate_bt2(path: Path) -> tuple[bool, str]:
    ...
    for index, row in enumerate(rows, start=2):
        ...
        if not _substantive_bt2_source_locator(row.get("source_locator")):
            return False, f"row {index}: source_locator is not substantive source evidence"
        if not _source_authority_valid(row.get("authority_class")):
            return False, f"row {index}: authority_class is not source/controlled authority"
        ...
        residual_decimal = _finite_decimal(row.get("mass_balance_residual"))
        ...
        if "tolerance" in fields and str(row.get("tolerance", "")).strip():
            tolerance_decimal = _finite_decimal(row.get("tolerance"))
    ...
    for cohort, expected_minimum in required_cohorts.items():
        ...
        for row in ordered:
            ...
            if residual_decimal is None:
                return False, (
                    f"{cohort[0]}/{cohort[1]} {row['qcell_id']}: "
                    "mass_balance_residual must be finite"
                )
            residual_abs = residual_decimal.copy_abs()
            if residual_abs == Decimal("0"):
                continue
            ...
            tolerance_decimal = _finite_decimal(tolerance_raw)
            if tolerance_decimal is None or tolerance_decimal < 0:
                return False, (
                    f"{cohort[0]}/{cohort[1]} {row['qcell_id']}: "
                    "source-defined tolerance must be finite and >= 0"
                )
            if residual_abs > tolerance_decimal:
                return False, (
                    f"{cohort[0]}/{cohort[1]} {row['qcell_id']}: "
...
def validate_bt3(path: Path, geometry_path: Path) -> tuple[bool, str]:
    """Validate the full W57 source-bound spatial thermal/property field."""
    geometry_ok, geometry_reason = validate_bt1(geometry_path)
    if not geometry_ok:
        return False, f"selected-current geometry unavailable for BT3 coverage: {geometry_reason}"
    ...
    if segment_ids != expected_segment_ids:
        missing_segments = sorted(expected_segment_ids - segment_ids)
        extra_segments = sorted(segment_ids - expected_segment_ids)
        return False, (
            "thermal/property segment coverage mismatch; "
            f"missing={missing_segments}, extra={extra_segments}"
        )
...
def validate_bt4(path: Path) -> tuple[bool, str]:
    ...
    if not _psv_locator_matches(
        psv.get("source_locator"),
        tag=psv.get("tag"),
        set_pressure_bara=psv.get("set_pressure_bara"),
        reseat_pressure_bara=psv.get("reseat_pressure_bara"),
    ):
        return False, (
            "psv.source_locator must exactly bind boundary-delimited tag plus labeled "
            "set and reseat pressures"
        )
