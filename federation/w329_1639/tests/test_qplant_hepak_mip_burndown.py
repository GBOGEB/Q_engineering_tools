...
class HepakMIPBurndownTests(unittest.TestCase):
    ...
    def _evaluate_with_owner_attestation(
        self,
        root: Path,
        workbook_sha: str = "b" * 64,
    ):
        with patch.dict(
            os.environ,
            {
                "QPS_HEPAK_OWNER_SOURCE_SHA256": workbook_sha,
                "QPS_HEPAK_OWNER_SOURCE_WORKBOOK": "CH15_HEPAK_QPLANT_Runtime_v0_1.xlsx",
            },
            clear=False,
        ):
    ...
    def test_repo_local_active_anchor_without_owner_attestation_does_not_convert_bt0(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self._write_valid_hepak_receipt(root, with_active_anchor=True)
            result = evaluate(root)
            self.assertFalse(result["blockers"]["BT0_HEPAK"]["converted"])
            self.assertIn("owner-controlled HEPAK SHA256 attestation", result["blockers"]["BT0_HEPAK"]["reason"])

    def test_active_anchor_plus_owner_attestation_allows_contract_fixture(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self._write_valid_hepak_receipt(root, with_active_anchor=True)
            result = self._evaluate_with_owner_attestation(root)
