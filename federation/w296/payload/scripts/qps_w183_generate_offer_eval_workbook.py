...
def build(
    ...
    for r, offer in enumerate(EXPECTED_OFFERS, start=2):
        ...
        calc.append([
            ...
            f'=IF(INPUT_AB!M{r}=0,"",IF(INPUT_AB!M{r}="","",IF(COUNT(INPUT_AB!D{r}:F{r})<3,IF(ISTEXT(INPUT_AB!D{r}),INPUT_AB!D{r},IF(ISTEXT(INPUT_AB!E{r}),INPUT_AB!E{r},IF(ISTEXT(INPUT_AB!F{r}),INPUT_AB!F{r},"MISSING"))),MIN(CONFIG!$B$2,SUM(INPUT_AB!D{r}:F{r})))))',
            f'=IF(INPUT_AB!M{r}=0,"",IF(OR(J{r}="",N{r}="",E{r}="",NOT(ISNUMBER(E{r}))),"",J{r}*N{r}*(1-E{r})))',
            f'=IF(INPUT_AB!O{r}=0,"",IF(INPUT_AB!O{r}="","",IF(COUNT(INPUT_AB!I{r}:K{r})<3,IF(ISTEXT(INPUT_AB!I{r}),INPUT_AB!I{r},IF(ISTEXT(INPUT_AB!J{r}),INPUT_AB!J{r},IF(ISTEXT(INPUT_AB!K{r}),INPUT_AB!K{r},"MISSING"))),MIN(CONFIG!$B$2,SUM(INPUT_AB!I{r}:K{r})))))',
            f'=IF(INPUT_AB!O{r}=0,"",IF(OR(K{r}="",O{r}="",G{r}="",NOT(ISNUMBER(G{r}))),"",K{r}*O{r}*(1-G{r})))',
